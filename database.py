...
def get_open_damage_reports():
    ...
    for row in rows:
        ...
        if "Student:" in description and "Part:" in description:
            try:
                student_line = description.split("Student:")[1].split("Part:")[0].strip()
                report["display_name"] = student_line.split("(")[0].strip()
            except Exception:
                report["display_name"] = report["reported_by_name"]
        else:
            report["display_name"] = report["reported_by_name"]
...
def mark_report_solved(report_id):
    execute_db(
        "UPDATE damage_reports SET status = 'resolved' WHERE report_id = ?",
        (report_id,)
    )
